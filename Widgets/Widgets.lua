...
if class then
	accentColor.t[1], accentColor.t[2], accentColor.t[3], accentColor.s =
...
function Cell.CreateSeparator(text, parent, width, color)
	if not color then
		color = { t = { accentColor.t[1], accentColor.t[2], accentColor.t[3], 0.777 }, s = accentColor.s }
	end
	if not width then
...
function Cell.CreateMovableFrame(title, name, width, height, frameStrata, frameLevel, notUserPlaced)
	...
	header.closeBtn = Cell.CreateButton(
		header,
		"X",
		"red",
		{ 20, 20 },
		false,
		false,
		"CELL_WRATH_FONT_WIDGET_TITLE",
		"CELL_WRATH_FONT_WIDGET_TITLE_DISABLE"
	)
...
function Cell.ChangeSizeWithAnimation(frame, targetWidth, targetHeight, step, startFunc, endFunc, repoint)
	...
	animationTimer = C_Timer.NewTicker(0.025, function()
		...
		if currentWidth == targetWidth and currentHeight == targetHeight then
			...
			if repoint then
				P.PixelPerfectPoint(frame)
			end -- already point to another frame
		end
	end)
end
...
function Cell.CreateButton(
	...
	if buttonColor ~= "none" then
		b:SetScript("OnEnter", function(self)
			-- For accent colors, dynamically get current accentColor
			if self.isAccentColor and self.accentHoverAlpha then
				self:SetBackdropColor(accentColor.t[1], accentColor.t[2], accentColor.t[3], self.accentHoverAlpha)
			else
		...
		b:SetScript("OnLeave", function(self)
			-- For accent colors, dynamically get current accentColor
			if self.isAccentColor and self.accentAlpha then
				self:SetBackdropColor(accentColor.t[1], accentColor.t[2], accentColor.t[3], self.accentAlpha)
			else
...
function Cell.CreateButtonGroup(buttons, onClick, func1, func2, onEnter, onLeave)
	-- Helper function to get the correct hover color (dynamic for accent colors)
	local function GetHoverColor(b)
		if b.isAccentColor and b.accentHoverAlpha then
			return { accentColor.t[1], accentColor.t[2], accentColor.t[3], b.accentHoverAlpha }
		else
			return b.hoverColor
		end
	end

	-- Helper function to get the correct base color (dynamic for accent colors)
	local function GetBaseColor(b)
		if b.isAccentColor and b.accentAlpha then
			return { accentColor.t[1], accentColor.t[2], accentColor.t[3], b.accentAlpha }
		else
			return b.color
		end
	end
	...
	local function HighlightButton(id)
		for _, b in pairs(buttons) do
			if id == b.id then
				b:SetBackdropColor(unpack(GetHoverColor(b)))
				b:SetScript("OnEnter", function()
					if b.ShowTooltip then
						b.ShowTooltip(b)
					end
					if onEnter then
						onEnter(b)
				...
				b:SetBackdropColor(unpack(GetBaseColor(b)))
				b:SetScript("OnEnter", function()
					if b.ShowTooltip then
						b.ShowTooltip(b)
					end
...
function Cell.CreateCheckButton(parent, label, onClick, ...)
	...
	cb:SetScript("OnClick", function(self)
		PlaySound(
			self:GetChecked() and SOUNDKIT.IG_MAINMENU_OPTION_CHECKBOX_ON or SOUNDKIT.IG_MAINMENU_OPTION_CHECKBOX_OFF
		)
		if cb.onClick then
...
function Cell.CreateEditBox(parent, width, height, isTransparent, isMultiLine, isNumeric, fontObj)
	...
	if not isTransparent then
		Cell.StylizeFrame(eb, { 0.115, 0.115, 0.115, 0.9 })
	end
...
function Cell.CreateScrollEditBox(parent, onTextChanged, scrollStep)
	...
	frame.eb:SetScript("OnEnterPressed", function(self)
		self:Insert("\n")
	end)
...
function Cell.CreateSlider(
	...
	currentEditBox:SetScript("OnEnterPressed", function(self)
		...
		if value then
			if value < slider.low then
				value = slider.low
			end
			if value > slider.high then
				value = slider.high
			end
			self:SetText(value)
			slider:SetValue(value)
			if slider.onValueChangedFn then
				slider.onValueChangedFn(value)
			end
			if slider.afterValueChangedFn then
	...
	slider:SetScript("OnValueChanged", function(self, value, userChanged)
		...
		if userChanged and slider.onValueChangedFn then
			slider.onValueChangedFn(value)
		end
	end)
...
function Cell.CreateSwitch(parent, size, leftText, leftValue, rightText, rightValue, func)
	...
	function switch:SetSelected(value, runFunc)
		...
		if func and runFunc then
			func(switch.selectedValue)
		end
	end
...
function Cell.CreateStatusBar(name, parent, width, height, maxValue, smooth, func, showText, texture, color)
	...
	if not color then
		color = { accentColor.t[1], accentColor.t[2], accentColor.t[3], 1 }
	end
	if not texture then
		texture = Cell.vars.whiteTexture
	end
	...
	end -- Interface\SharedXML\SmoothStatusBar.lua
...
function Cell.CreateConfirmPopup(parent, width, text, onAccept, onReject, mask, hasEditBox, dropdowns)
	if not parent.confirmPopup then -- not init
		...
		Cell.StylizeFrame(
			parent.confirmPopup,
			{ 0.1, 0.1, 0.1, 0.95 },
			{ accentColor.t[1], accentColor.t[2], accentColor.t[3], 1 }
		)
...
function Cell.CreateDualPopupEditBox(parent, leftTip, rightTip, isNumeric, func)
	if not parent.dualPopupEditBox then
		...
		left:SetScript("OnEditFocusGained", function()
			left:HighlightText()
		end)
		left:SetScript("OnEditFocusLost", function()
			left:HighlightText(0, 0)
		end)
		left:SetScript("OnEscapePressed", function()
			f:Hide()
		end)
		...
		right:SetScript("OnEditFocusGained", function()
			right:HighlightText()
		end)
		right:SetScript("OnEditFocusLost", function()
			right:HighlightText(0, 0)
		end)
		right:SetScript("OnEscapePressed", function()
			f:Hide()
		end)
...
local function CreateItemButtons(items, itemTable, itemParent, level)
	itemParent:SetScript("OnHide", function(self)
		self:Hide()
	end)
	...
	for i, item in pairs(items) do
		...
		if item.children then
			...
			if not menu[level + 1] then
				...
				menu[level + 1] = Cell.CreateFrame(
					addonName .. "CascadingSubMenu" .. level,
					level == 0 and menu or menu[level],
					100,
					20
				)
...
function Cell.CreateScrollTextFrame(parent, s, timePerScroll, scrollStep, delayTime, noFadeIn)
	...
	if not timePerScroll then
		timePerScroll = 0.02
	end
	if not scrollStep then
		scrollStep = 1
	end
...
function Cell.CreateScrollFrame(parent, top, bottom, color, border)
	...
	local scrollFrame =
		CreateFrame("ScrollFrame", parent:GetName() and parent:GetName() .. "ScrollFrame" or nil, parent, nil)
	...
	scrollThumb:SetScript("OnMouseDown", function(self, button)
		...
		self:SetScript("OnUpdate", function(self)
			...
			local vs = (-newOffsetY / (scrollbar:GetHeight() - scrollThumb:GetHeight()))
	...
	local function LoadItems()
		...
		for i, item in pairs(menu.items) do
			...
			b:SetScript("OnClick", function()
				...
				list:Hide()
				if item.onClick then
					item.onClick(item.text, item.value, menu.id)
				end
			end)
...
function Cell.CreateBindingButton(parent, width)
	if not parent.bindingButton then
		...
		Cell.StylizeFrame(
			parent.bindingButton,
			{ 0.1, 0.1, 0.1, 1 },
			{ accentColor.t[1], accentColor.t[2], accentColor.t[3] }
		)

		parent.bindingButton.close = Cell.CreateButton(
			parent.bindingButton,
			"X",
			"red",
			{ 18, 18 },
		...
		parent.bindingButton:SetScript("OnMouseDown", function(self, key)
			...
			if parent.bindingButton.func then
				parent.bindingButton.func(GetModifier(), key)
			end
		end)
		...
		parent.bindingButton:SetScript("OnMouseWheel", function(self, key)
			parent.bindingButton:Hide()
			if parent.bindingButton.func then
				parent.bindingButton.func(GetModifier(), (key == 1) and "ScrollUp" or "ScrollDown")
			end
		end)
...
function Cell.CreateReceivingFrame(parent)
	...
	-- bar
	local progressBar = Cell.CreateStatusBar(
		"CellReceivingFrameBar",
		f,
		198,
		18,
		0,
		...
		nil,
		true,
		"Interface\\AddOns\\Cell_Wrath\\Media\\statusbar.tga",
		{ 0.7, 0.7, 0, 1 }
	)
	...
	function f:ShowFrame(type, playerName, name1, name2)
		...
		Cell.ChangeSizeWithAnimation(
			f,
			249,
			40 + lines * math.ceil(typeLabel:GetStringHeight() * 1.4),
			7,
			nil,
			function()
				typeLabel:Show()
			...
			timeout = C_Timer.NewTimer(10, function()
				if f:IsShown() and progressBar:GetValue() == 0 then
					...
					Cell.ChangeSizeWithAnimation(
						f,
						249,
						f.height + 15 + math.ceil(infoMsg:GetStringHeight()),
						7,
						nil,
						function()
							infoMsg:Show()
	...
	function f:ShowImport(status, received, func)
		...
		if status then
			if received["type"] == "Debuffs" then
				local isCompatible = type(received["version"]) == "number"
					and received["version"] >= Cell.MIN_DEBUFFS_VERSION
				...
				C_Timer.After(0.5, function()
					...
					Cell.ChangeSizeWithAnimation(
						f,
						249,
						f.height + 15 + math.ceil(dataText:GetStringHeight() + infoMsg:GetStringHeight()),
						7,
						nil,
						function()
							dataLabel:Show()
			...
			elseif received["type"] == "Layout" then
				local isCompatible = type(received["version"]) == "number"
					and received["version"] >= Cell.MIN_LAYOUTS_VERSION
