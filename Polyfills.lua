...
if not UnitGroupRolesAssigned then
	...
	function UnitGroupRolesAssigned(unit)
		...
		if roleDebugEnabled then
			...
			print(string.format("[Role Debug] %s -> %s (source: %s)", UnitName(unit) or unit, roleName, roleSource))
		end
...
do
	...
	if mt and mt.__index and type(mt.__index.CreateAnimation) == "function" and not mt.__index._CellFlipBookShim then
...
-------------------------------------------------
-- UnitAura safety wrapper for Cell internal use only
-- WotLK errors on nil/empty unit or nil index/name; retail is lenient.
-- NOTE: We no longer override global UnitAura to avoid conflicts with other addons like ElvUI
-------------------------------------------------
-- Store original function reference for Cell to use
if not _G._CellOriginalUnitAura then
	_G._CellOriginalUnitAura = UnitAura
end

-- Cell-specific safe wrapper (don't override global)
if Cell then
	Cell.SafeUnitAura = function(unit, indexOrName, ...)
...
if Cell then
	Cell.UnitBuff = function(unit, index, filter)
		local name, rank, icon, count, debuffType, duration, expirationTime, caster, isStealable, shouldConsolidate, spellId =
			_G._CellOriginalUnitBuff(unit, index, filter)
		if not name then
			return nil
		end
	...
	Cell.UnitDebuff = function(unit, index, filter)
		local name, rank, icon, count, debuffType, duration, expirationTime, caster, isStealable, shouldConsolidate, spellId =
			_G._CellOriginalUnitDebuff(unit, index, filter)
		if not name then
			return nil
		end
		return name,
...
-- NOTE: Don't create C_NamePlate stub - it can break other addons like ElvUI
-- that have their own nameplate implementations
...
do
	...
	if ok and test then
		...
		if mt and mt.__index and type(mt.__index.SetFrameRef) == "function" and not mt.__index._CellSetFrameRefShim then
	...
	function GetClassColor(classFile)
		...
		if color then
			return color.r,
				color.g,
				color.b,
				string.format("%02x%02x%02x%02x", 255, color.r * 255, color.g * 255, color.b * 255)
		end
...
do
	...
	proxyFrame:SetScript("OnEvent", function(self, event)
		...
		lastFireTime = now
		local count = 0
		for _ in pairs(groupRosterFrames) do
			count = count + 1
		end
...
do
	...
	if mt and mt.__index and not mt.__index._CellDebugHooked then
		...
		mt.__index.SetFont = function(self, path, size, flags)
			...
			local isTargetAddon = parentName:match("Quartz")
				or parentName:match("NotPlater")
				or parentName:match("XPerl")
				or selfName:match("Quartz")
				or selfName:match("NotPlater")
				or selfName:match("XPerl")

			if isTargetAddon and caller:match("Cell_Wrath") then
				print(
					string.format(
						"|cFFFF0000[Cell Debug]|r Cell modifying %s addon!",
						parentName:match("Quartz") and "Quartz"
							or parentName:match("NotPlater") and "NotPlater"
							or "XPerl"
					)
				)
		...
		mt.__index.SetFontObject = function(self, fontObj)
			...
			if isTargetAddon and caller:match("Cell_Wrath") then
				print(
					string.format(
						"|cFFFF0000[Cell Debug]|r Cell SetFontObject on %s addon!",
						parentName:match("Quartz") and "Quartz"
							or parentName:match("NotPlater") and "NotPlater"
							or "XPerl"
					)
		...
		SlashCmdList["CELLDEBUG"] = function(msg)
			if msg == "fonts" then
				...
				for i = math.max(1, #debugLog - 20), #debugLog do
					local e = debugLog[i]
					print(string.format("[%.2f] %s.%s size=%s", e.time, e.parent, e.self, tostring(e.size)))
...
do
	...
	function PatchCreateScrollFrame()
		if not Cell or not Cell.CreateScrollFrame then
			return
		end
		if Cell._ScrollFramePatchedForBindings then
			return
		end
		...
		Cell.CreateScrollFrame = function(parent, top, bottom, color, border)
			...
			if parent:GetName() == "ClickCastingsTab_BindingsFrame" then
				...
				if scroll then
					...
					if scroll.content and scroll.content.EnableMouse then
						scroll.content:EnableMouse(true)
					end
				end
			end
	...
	function PatchBindingListButton()
		if not Cell or not Cell.CreateBindingListButton then
			return
		end
		if Cell._BindingListButtonPatched then
			return
		end
		...
		Cell.CreateBindingListButton = function(parent, modifier, bindKey, bindType, bindAction)
			...
			if b.keyGrid then
				b.keyGrid:SetFrameLevel(level)
			end
			if b.typeGrid then
				b.typeGrid:SetFrameLevel(level)
			end
			if b.actionGrid then
				b.actionGrid:SetFrameLevel(level)
			end
	...
	local function PatchGetClickCastingSpellList()
		if not Cell or not Cell.funcs or not Cell.funcs.GetClickCastingSpellList then
			return
		end
		if Cell._GetClickCastingSpellListPatched then
			return
		end
	...
	local function PatchAnimationSystem()
		if Cell._AnimationSystemPatched then
			return
		end
		...
		mt.__index.CreateAnimation = function(self, type, name, inherits)
			...
			if not self._cellAnimations then
				self._cellAnimations = {}
			end
				...
				anim._ApplyFromState = function(self)
					...
					if self._fromX or self._fromY then
						...
						if region and region.SetScale then
							...
							if startScale <= 0.001 then
								startScale = 0.001
							end -- Prevent 0 scale issues
	...
	local function PatchLibCustomGlow()
		...
		if Cell._LibCustomGlowPatched then
			return
		end
			...
			for k = 1, 4 do
				...
				for i = 1, self.info.N do
					...
					if self.textures[texIndex] then
						local position = (self.info.space * i + self.info.perimeter * self.timer[k])
							% self.info.perimeter
						if position > self.info.bottomlim then
							self.textures[texIndex]:SetPoint(
								"CENTER",
								self,
								"BOTTOMRIGHT",
								-position + self.info.bottomlim,
								0
							)
