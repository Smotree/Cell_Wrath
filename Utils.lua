...
local separatedHeaders = {
	"CellRaidFrameHeader1",
	"CellRaidFrameHeader2",
	"CellRaidFrameHeader3",
	"CellRaidFrameHeader4",
	"CellRaidFrameHeader5",
	"CellRaidFrameHeader6",
	"CellRaidFrameHeader7",
	"CellRaidFrameHeader8",
}
...
function F.IterateAllUnitButtons(func, updateCurrentGroupOnly, updateQuickAssists, skipShared)
	...
	-- group pet
	if
		not updateCurrentGroupOnly
		or (updateCurrentGroupOnly and Cell.vars.groupType == "raid")
		or (updateCurrentGroupOnly and Cell.vars.groupType == "party")
	then
...
function F.GetUnitButtonByUnit(unit, getSpotlights, getQuickAssist)
	...
	if Cell.vars.groupType == "raid" then
		if Cell.vars.inBattleground == 5 then
			normal = Cell.unitButtons.raid.units[unit]
				or Cell.unitButtons.npc.units[unit]
				or Cell.unitButtons.arena[unit]
		else
			normal = Cell.unitButtons.raid.units[unit]
				or Cell.unitButtons.npc.units[unit]
				or Cell.unitButtons.pet.units[unit]
		end
...
function F.GetMarkEscapeSequence(index)
	...
	return string.format(
		"|TInterface\\TargetingFrame\\UI-RaidTargetingIcons:0:0:0:0:64:64:%d:%d:%d:%d|t",
		left * 64,
		right * 64,
		top * 64,
		bottom * 64
...
local function NormalizeClassToken(class)
	...
	if type(class) == "number" then
		return CLASS_ID_TO_TOKEN[class]
	end
	if CLASS_NAME_TO_TOKEN[class] then
		return CLASS_NAME_TO_TOKEN[class]
	end
end
	...
	if percent == 1 and Cell.vars.useFullColor then
		...
		elseif CellDB["appearance"]["barColor"][1] == "threshold3" then
			...
			if percent >= c[5] then
				...
				barR, barG, barB =
					F.ColorThreshold(percent, c[1], c[2], { r * 0.2, g * 0.2, b * 0.2 }, c[4], c[5], c[6])
			end
		else
	...
	if isDeadOrGhost and Cell.vars.useDeathColor then
		...
		elseif CellDB["appearance"]["lossColor"][1] == "threshold3" then
			...
			if isDeadOrGhost or percent <= c[4] then
				...
				lossR, lossG, lossB =
					F.ColorThreshold(percent, { r * 0.2, g * 0.2, b * 0.2 }, c[2], c[3], c[4], c[5], c[6])
			end
		else
...
function F.GetTargetUnitID(target)
	...
	if not F.UnitInGroup(target) then
		return
	end
...
function F.IsFriend(unitFlags)
	if not unitFlags or type(unitFlags) ~= "number" then
		return false
	end
	return (bit.band(unitFlags, OBJECT_AFFILIATION_MINE) ~= 0)
		or (bit.band(unitFlags, OBJECT_AFFILIATION_RAID) ~= 0)
		or (bit.band(unitFlags, OBJECT_AFFILIATION_PARTY) ~= 0)
end
...
function F.HasPermission(isPartyMarkPermission)
	if isPartyMarkPermission and IsInGroup() and not IsInRaid() then
		return true
	end
...
function Cell.GetUnitFramesForLGF(unit, frames, priorities)
	...
	local normal, spotlights, quickAssist =
		F.GetUnitButtonByUnit(unit, spotlightPriorityEnabled, quickAssistPriorityEnabled)
...
local function GetResult1()
	...
	return "UnitID: "
		.. (F.GetTargetUnitID("target") or "target")
		.. "\n|cffffff00F.IsInRange:|r "
		.. (F.IsInRange("target") and "true" or "false")
		.. "\nUnitInRange: "
		.. (checked and "checked" or "unchecked")
		.. " "
		.. (inRange and "true" or "false")
		.. "\nUnitIsVisible: "
		.. (UnitIsVisible("target") and "true" or "false")
		.. "\n\nUnitCanAssist: "
		.. (UnitCanAssist("player", "target") and "true" or "false")
		.. "\nUnitCanCooperate: "
		.. (UnitCanCooperate("player", "target") and "true" or "false")
		.. "\nUnitCanAttack: "
		.. (UnitCanAttack("player", "target") and "true" or "false")
		.. "\n\nUnitIsConnected: "
		.. (UnitIsConnected("target") and "true" or "false")
		.. "\nUnitInSamePhase: "
		.. (UnitInSamePhase("target") and "true" or "false")
		.. "\nUnitIsDead: "
		.. (UnitIsDead("target") and "true" or "false")
		.. "\n\nspell_friend: "
		.. (spell_friend and (spell_friend .. " " .. (UnitInSpellRange(spell_friend, "target") and "true" or "false")) or "none")
		.. "\nspell_dead: "
		.. (spell_dead and (spell_dead .. " " .. (UnitInSpellRange(spell_dead, "target") and "true" or "false")) or "none")
		.. "\nspell_pet: "
		.. (spell_pet and (spell_pet .. " " .. (UnitInSpellRange(spell_pet, "target") and "true" or "false")) or "none")
		.. "\nspell_harm: "
		.. (
			spell_harm and (spell_harm .. " " .. (UnitInSpellRange(spell_harm, "target") and "true" or "false"))
			or "none"
		)
end
...
local function GetResult2()
	if UnitCanAttack("player", "target") then
		return "IsItemInRange: "
			.. (IsItemInRange(harmItems[playerClass], "target") and "true" or "false")
			.. "\nCheckInteractDistance(28y): "
			.. (CheckInteractDistance("target", 4) and "true" or "false")
	else
		return "IsItemInRange: "
			.. (InCombatLockdown() and "notAvailable" or (IsItemInRange(harmItems[playerClass], "target") and "true" or "false"))
			.. "\nCheckInteractDistance(28y): "
			.. (InCombatLockdown() and "notAvailable" or (CheckInteractDistance("target", 4) and "true" or "false"))
	end
end
