...
function F.UpdateLayout(layoutGroupType)
	if InCombatLockdown() then
		...
		if
			Cell.vars.currentLayoutTable
			and Cell.vars.currentLayoutTable["main"]
			and Cell.vars.currentLayoutTable["main"]["combineGroups"]
		then
...
function eventFrame:ADDON_LOADED(arg1)
	if arg1 == "Cell_Wrath" or arg1 == "Cell" then
		...
		if type(CellCharacterDB) ~= "table" then
			CellCharacterDB = {}
		end
		...
		if type(CellDB["optionsFramePosition"]) ~= "table" then
			CellDB["optionsFramePosition"] = {}
		end
		...
		if type(CellDB["customTextures"]) ~= "table" then
			CellDB["customTextures"] = {}
		end

		if type(CellDB["snippets"]) ~= "table" then
			CellDB["snippets"] = {}
		end
		...
		if type(CellDB["general"]) ~= "table" then
			CellDB["general"] = {
				...
				-- Don't hide Raid Manager on WotLK - it contains World Markers buttons
				["hideBlizzardRaidManager"] = not (Cell.isWrath or Cell.isVanilla),
				["locked"] = false,
				["fadeOut"] = false,
		...
		-- aoeHealings ----------------------------------------------------------------------------
		if type(CellDB["aoeHealings"]) ~= "table" then
			CellDB["aoeHealings"] = { ["disabled"] = {}, ["custom"] = {} }
		end

		-- defensives/externals -------------------------------------------------------------------
		if type(CellDB["defensives"]) ~= "table" then
			CellDB["defensives"] = { ["disabled"] = {}, ["custom"] = {} }
		end
		if type(CellDB["externals"]) ~= "table" then
			CellDB["externals"] = { ["disabled"] = {}, ["custom"] = {} }
		end

		-- crowdControls --------------------------------------------------------------------------
		if type(CellDB["crowdControls"]) ~= "table" then
			CellDB["crowdControls"] = { ["disabled"] = {}, ["custom"] = {} }
		end

		-- raid debuffs ---------------------------------------------------------------------------
		if type(CellDB["raidDebuffs"]) ~= "table" then
			CellDB["raidDebuffs"] = {}
		end
		-- CellDB["raidDebuffs"] = {
		--     [instanceId] = {
		...
		if not CellDB["revise"] then
			CellDB["firstRun"] = true
		end
		F.Revise()
		F.CheckWhatsNew()
		F.RunSnippets()
