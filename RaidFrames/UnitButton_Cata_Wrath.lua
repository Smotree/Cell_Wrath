...
-- LibHealComm-4.0 for heal prediction
...
local function UpdateIndicatorParentVisibility(b, indicatorName, enabled)
	if
		not (
			...
			or indicatorName == "privateAuras"
			or indicatorName == "defensiveCooldowns"
			or indicatorName == "externalCooldowns"
			or indicatorName == "allCooldowns"
			or indicatorName == "crowdControls"
			or indicatorName == "dispels"
			or indicatorName == "missingBuffs"
		)
	then
		return
	end
...
local function UpdateIndicators(layout, indicatorName, setting, value, value2)
	...
	if not indicatorName then -- init
		...
		elseif setting == "size-width" then
			F.IterateAllUnitButtons(function(b)
				local indicator = b.indicators[indicatorName]
				local height = (indicator.configs and indicator.configs["size"] and indicator.configs["size"][2])
					or value[2]
					or indicator:GetHeight()
					or 0
		...
		elseif
			setting == "debuffBlacklist"
			or setting == "dispelBlacklist"
			or setting == "defensives"
			or setting == "externals"
			or setting == "bigDebuffs"
			or setting == "debuffTypeColor"
		then
			F.IterateAllUnitButtons(function(b)
				UnitButton_UpdateAuras(b)
			end, true)
	...
	for i = 1, 40 do
		...
		if duration then
			if Cell.vars.iconAnimation == "duration" then
				local timeIncreased = self._debuffs_cache[auraInstanceID]
						and (expirationTime - self._debuffs_cache[auraInstanceID] >= 0.5)
					or false
				local countIncreased = self._debuffs_count_cache[auraInstanceID]
						and (count > self._debuffs_count_cache[auraInstanceID])
					or false
				refreshing = timeIncreased or countIncreased
			elseif Cell.vars.iconAnimation == "stack" then
				refreshing = self._debuffs_count_cache[auraInstanceID]
						and (count > self._debuffs_count_cache[auraInstanceID])
					or false
			else
				refreshing = false
			...
			-- crowdControls
			if
				enabledIndicators["crowdControls"]
				and I.IsCrowdControls(name, spellId)
				and ccFound <= indicatorNums["crowdControls"]
			then
				-- start, duration, debuffType, texture, count, refreshing
				self.indicators.crowdControls[ccFound]:SetCooldown(
					expirationTime - duration,
					duration,
					debuffType,
					icon,
		...
		for i = 1, indicatorNums["raidDebuffs"] do
			...
			if index then
				local name, icon, count, debuffType, duration, expirationTime, source, isStealable, nameplateShowPersonal, spellId =
					Cell.UnitDebuff(unit, self._debuffs_raid[i])
				if name then
	...
	if enabledIndicators["debuffs"] then
		...
		for debuffIndex, refreshing in pairs(self._debuffs_big) do
			local name, icon, count, debuffType, duration, expirationTime, _, _, _, spellId =
				Cell.UnitDebuff(unit, debuffIndex)
...
local function UnitButton_UpdateBuffs(self)
	...
	for i = 1, 40 do
		-- name, icon, count, debuffType, duration, expirationTime, source, isStealable, nameplateShowPersonal, spellId, canApplyAura, isBossDebuff, castByPlayer, nameplateShowAll, timeMod, ...
		local name, icon, count, debuffType, duration, expirationTime, source, _, _, spellId, _, _, _, _, _, arg16 =
			Cell.UnitBuff(unit, i)
		if not name then
			break
		end
		...
		if duration then
			if Cell.vars.iconAnimation == "duration" then
				local timeIncreased = self._buffs_cache[auraInstanceID]
						and (expirationTime - self._buffs_cache[auraInstanceID] >= 0.5)
					or false
				local countIncreased = self._buffs_count_cache[auraInstanceID]
						and (count > self._buffs_count_cache[auraInstanceID])
					or false
				refreshing = timeIncreased or countIncreased
			elseif Cell.vars.iconAnimation == "stack" then
				refreshing = self._buffs_count_cache[auraInstanceID]
						and (count > self._buffs_count_cache[auraInstanceID])
					or false
			else
				refreshing = false
			...
			-- defensiveCooldowns
			if
				enabledIndicators["defensiveCooldowns"]
				and I.IsDefensiveCooldown(name, spellId)
				and defensiveFound <= indicatorNums["defensiveCooldowns"]
			then
				-- start, duration, debuffType, texture, count, refreshing
				self.indicators.defensiveCooldowns[defensiveFound]:SetCooldown(
					expirationTime - duration,
					duration,
					nil,
					icon,
			...
			-- externalCooldowns
			if
				enabledIndicators["externalCooldowns"]
				and I.IsExternalCooldown(name, spellId, source, unit)
				and externalFound <= indicatorNums["externalCooldowns"]
			then
				-- start, duration, debuffType, texture, count, refreshing
				self.indicators.externalCooldowns[externalFound]:SetCooldown(
					expirationTime - duration,
					duration,
					nil,
					icon,
			...
			-- allCooldowns
			if
				enabledIndicators["allCooldowns"]
				and (I.IsExternalCooldown(name, spellId, source, unit) or I.IsDefensiveCooldown(name, spellId))
				and allFound <= indicatorNums["allCooldowns"]
			then
				-- start, duration, debuffType, texture, count, refreshing
				self.indicators.allCooldowns[allFound]:SetCooldown(
					expirationTime - duration,
					duration,
					nil,
					icon,
			...
			if
				enabledIndicators["powerWordShield"]
				and IsPowerWordShield(spellId, name)
				and (not indicatorBooleans["powerWordShield"] or source == "player")
			then
				pwsFound = true
...
local function UnitButton_UpdatePowerStates(self)
	...
	if self.states.powerMax <= 0 then
		self.states.powerMax = 1
	end
end
...
ShouldShowPowerText = function(b)
	if not enabledIndicators["powerText"] then
		return
	end
...
ShouldShowPowerBar = function(b)
	...
	if not b.powerSize or b.powerSize == 0 then
		return
	end
...
local function ShowPowerBar(b)
	...
	if b.orientation == "horizontal" or b.orientation == "vertical_health" then
		...
		P.Point(
			...
			"BOTTOMRIGHT",
			-CELL_BORDER_SIZE,
			b.powerSize + CELL_BORDER_SIZE * 2
		)
...
local function UnitButton_UpdateTarget(self)
	...
	if UnitIsUnit(unit, "target") then
		if highlightEnabled then
			self.widgets.targetHighlight:Show()
		end
	else
...
local function UnitButton_FinishReadyCheck(self)
	if not enabledIndicators["readyCheckIcon"] then
		return
	end
...
UnitButton_UpdatePowerMax = function(self)
	if not (self._shouldShowPowerBar and self.states.powerMax) then
		return
	end
...
UnitButton_UpdatePower = function(self)
	if not (self._shouldShowPowerBar and self.states.power) then
		return
	end
...
UnitButton_UpdatePowerType = function(self)
	if not self._shouldShowPowerBar then
		return
	end
...
local function UnitButton_UpdateHealPrediction(self)
	...
	-- Try LibHealComm-4.0 first (more reliable on Sirus)
	if HealComm then
	...
	-- Fallback to native API if HealComm returns 0
	if value == 0 then
...
local function UnitButton_UpdateCombatIcon(self)
	if not enabledIndicators["combatIcon"] then
		return
	end
...
UnitButton_UpdateNameTextColor = function(self)
	...
	if enabledIndicators["nameText"] then
		if
			indicatorColors["nameText"][1] == "class_color"
			or not UnitIsConnected(unit)
			or (UnitIsPlayer(unit) and UnitIsCharmed(unit))
			or self.states.inVehicle
		then
...
UnitButton_UpdateHealthColor = function(self)
	...
	if UnitIsPlayer(unit) then -- player
		...
		elseif self.states.inVehicle then
			barR, barG, barB, lossR, lossG, lossB = F.GetHealthBarColor(
				self.states.healthPercent,
				self.states.isDeadOrGhost or self.states.isDead,
				0,
				1,
				0.2
			)
			...
			barR, barG, barB, lossR, lossG, lossB = F.GetHealthBarColor(
				self.states.healthPercent,
				self.states.isDeadOrGhost or self.states.isDead,
				F.GetClassColor(self.states.class)
			)
		end
	elseif F.IsPet(self.states.guid) then -- pet
		barR, barG, barB, lossR, lossG, lossB =
			F.GetHealthBarColor(self.states.healthPercent, self.states.isDeadOrGhost or self.states.isDead, 0.5, 0.5, 1)
	else -- npc
		barR, barG, barB, lossR, lossG, lossB =
			F.GetHealthBarColor(self.states.healthPercent, self.states.isDeadOrGhost or self.states.isDead, 0, 1, 0.2)
	end
	...
	if Cell.loaded and CellDB["appearance"]["healPrediction"][2] then
		self.widgets.incomingHeal:SetVertexColor(
			CellDB["appearance"]["healPrediction"][3][1],
			CellDB["appearance"]["healPrediction"][3][2],
			CellDB["appearance"]["healPrediction"][3][3],
			CellDB["appearance"]["healPrediction"][3][4]
		)
	else
	...
	if self.states.totalAbsorbs > 0 then
		...
		if enabledIndicators["shieldBar"] then
			if indicatorBooleans["shieldBar"] then
				-- onlyShowOvershields
				local overshieldPercent = (self.states.totalAbsorbs + self.states.health - self.states.healthMax)
					/ self.states.healthMax
...
local function UnitButton_UpdatePowerWordShield(self, current, max, resetMax)
	if not enabledIndicators["powerWordShield"] then
		return
	end
...
cleu:SetScript("OnEvent", function(_, _, ...)
	...
	if subEvent == "SPELL_HEAL" then
		if spellId == 56160 then -- Glyph of Power Word: Shield
			...
			if not absorbInfos[destGUID] then
				absorbInfos[destGUID] = {}
			end
	...
	elseif subEvent == "SPELL_AURA_APPLIED" or subEvent == "SPELL_AURA_REFRESH" then
		if spellName and ABSORB_SPELLS[spellName] then
			...
			if
				spellName == PWS_NAME
				and lastGlyphHeal[destGUID]
				and lastHealTimeStamp[destGUID]
				and (timestamp - lastHealTimeStamp[destGUID] < 0.5)
			then
				if not absorbInfos[destGUID] then
					absorbInfos[destGUID] = {}
				end
		...
		elseif spellId == 64411 and sourceGUID == Cell.vars.playerGUID then
			...
			if lastHealAmount then
				if not absorbInfos[lastHealGUID] then
					absorbInfos[lastHealGUID] = {}
				end
	...
	elseif subEvent == "SWING_DAMAGE" then
		...
		if absorbed and absorbed > 0 and absorbInfos[destGUID] then
			...
			for spellName, amount in pairs(absorbInfos[destGUID]) do
				...
				if absorbInfos[destGUID][spellName] <= 0 then
					absorbInfos[destGUID][spellName] = nil
				end
...
cleuHealthUpdater:SetScript("OnEvent", function(self, event, ...)
	...
	if not F.IsFriend(destFlags) then
		return
	end
...
UnitButton_UpdateAll = function(self)
	...
	if Cell.loaded then
		if self._powerUpdateRequired then
			...
			-- Always update power values even when _powerUpdateRequired is not set
			-- This fixes issue #31 where power bar may not show full
			if self._shouldShowPowerBar then
...
local function UnitButton_RegisterEvents(self)
	...
	-- WotLK/Sirus specific max power events
...
local function UnitButton_OnEvent(self, event, unit, ...)
	...
	if
		...
		and (
			...
			or UnitIsUnit(unit, self.states.displayedUnit)
		)
	then
...
local function UnitButton_OnAttributeChanged(self, name, value)
	if name == "unit" and not self:GetAttribute("oldUnit") then
		Cell.funcs.Debug(
			"|cffaaaa00[OnAttributeChanged] name:|r",
			name,
			"|cffaaaa00value:|r",
			value,
			"|cffaaaa00self.states.unit:|r",
			self.states.unit,
			"|cffaaaa00Button:|r",
			self:GetName()
		)
		...
		if type(value) == "string" then
			...
			if string.find(value, "raid") then
			...
			if string.find(value, "party") or value == "player" or value == "pet" or string.find(value, "partypet") then
				if UnitExists(value) then
					...
					Cell.funcs.Debug(
						"|cff00ffff[OnAttributeChanged] Populated party unit:|r",
						value,
						"Button:",
						self:GetName()
					)
					...
					Cell.funcs.Debug(
						"|cffff0000[OnAttributeChanged] Cleared invalid party unit:|r",
						value,
						"Button:",
						self:GetName()
					)
...
local function UnitButton_OnEnter(self)
	...
	if highlightEnabled then
		self.widgets.mouseoverHighlight:Show()
	end
...
local function UnitButton_OnTick(self)
		...
		if self.states.unit and self.states.displayedUnit then
			...
			if guid and guid ~= self.__unitGuid then
				...
				if UnitIsPlayer(self.states.unit) then
					...
					local name = GetUnitName(self.states.unit, true)
					if
						(name and self.__nameRetries and self.__nameRetries >= 4)
						or (name and name ~= UNKNOWN and name ~= UNKNOWNOBJECT)
					then
	...
	elseif self._updateRequired and not self._loggedPendingUpdate then
		...
		F.Debug(
			"Pending update but indicators not ready:",
			self:GetName(),
			"unit",
			self.states.unit or "nil",
			"guid",
			self.states.guid or "nil"
		)
	end
...
function B.UpdateHighlightSize(button)
	...
	if not targetHighlight or not mouseoverHighlight then
		return
	end
