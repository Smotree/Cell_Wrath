...
local function CreateRTPane()
	rtPane = Cell.CreateTitledPane(
		Cell.frames.utilitiesTab,
		L["Raid Tools"] .. " |cFF777777" .. L["only in group"],
		422,
		167
	)
	...
	reportCB:HookScript("OnEnter", function()
		...
		CellTooltip:AddLine(
			"|cffffffff" .. L["Use |cFFFFB5C5/cell report X|r to set the number of reports during a raid encounter"]
		)
		CellTooltip:AddLine(
			"|cffffffff"
				.. L["Current"]
				.. ": |cFFFFB5C5"
				.. (
					CellDB["tools"]["deathReport"][2] == 0 and L["all"]
					or string.format(L["first %d"], CellDB["tools"]["deathReport"][2])
				)
		)
		CellTooltip:Show()
	end)
	...
	buffCB = Cell.CreateCheckButton(
		...
		Cell.isRetail and L["|cffffb5c5Left-Click:|r cast the spell"]
			or "|cffffb5c5(Shift)|r " .. L["|cffffb5c5Left-Click:|r cast the spell"],
	...
	-- ready & pull
	readyPullCB = Cell.CreateCheckButton(
		rtPane,
		L["ReadyCheck and PullTimer buttons"],
		function(checked, self)
		...
		L["ReadyCheck and PullTimer buttons"],
		L["Only show when you have permission to do this"],
		L["readyCheckTips"],
		L["pullTimerTips"]
	)
	...
	marksDropdown:SetItems({
		...
		},
		{
			...
			["value"] = "world_h",
			-- Works on Sirus via /wm macro command
			["onClick"] = function()
