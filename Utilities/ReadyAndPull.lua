...
buttonsFrame.moverText:SetPoint("LEFT", 2, 0)
buttonsFrame.moverText:SetPoint("RIGHT", -2, 0)
buttonsFrame.moverText:SetWordWrap(false)
buttonsFrame.moverText:SetText(L["Mover"])
buttonsFrame.moverText:Hide()

local function ShowMover(show)
	if show then
		if not CellDB["tools"]["readyAndPull"][1] then
			return
		end
...
local function Start(sec, sendToChat)
	...
	pullTicker = C_Timer.NewTicker(1, function()
		...
		elseif pullBtn.sec <= -1 then
			...
			-- Stop ticker to prevent further negative values
			if pullTicker then
				pullTicker:Cancel()
				pullTicker = nil
			end
...
local function UpdateTools(which)
	...
	if not which or which == "readyAndPull" then
		if not pullBtn.tex then
			CreateTexture(pullBtn, "Interface\\AddOns\\Cell_Wrath\\Media\\Icons\\pull")
		end
		if not readyBtn.tex then
			CreateTexture(readyBtn, "Interface\\AddOns\\Cell_Wrath\\Media\\Icons\\ready")
		end
		...
		elseif CellDB["tools"]["readyAndPull"][3][1] == "bw" then
			...
			if Cell.isRetail then
				...
				pullBtn:SetScript("OnMouseUp", function(self, button)
					if button == "LeftButton" then
						SendChatMessage(
							L["Pull in %d sec"]:format(CellDB["tools"]["readyAndPull"][3][2]),
							IsInRaid() and "RAID_WARNING" or "PARTY"
						)
