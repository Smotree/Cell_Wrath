...
-- Check if player knows the spell (any rank)
local function PlayerKnowsSpell(spellId)
	-- Try IsSpellKnown first (available in some versions)
	if IsSpellKnown then
		return IsSpellKnown(spellId)
	end
	-- Fallback: check if GetSpellCooldown works (returns nil if spell unknown)
...
do
	local function Handle(buff, t, k)
		...
		if myClass == t["provider"] and not buffsProvidedByMe[buff] then
			buffsProvidedByMe[buff] = { name, icon, spellId }
		end
	end
...
buffTrackerFrame.moverText:SetPoint("LEFT", 2, 0)
buffTrackerFrame.moverText:SetPoint("RIGHT", -2, 0)
buffTrackerFrame.moverText:SetWordWrap(false)
...
local function ShowMover(show)
	if show then
		if not CellDB["tools"]["buffTracker"][1] then
			return
		end
...
do
	for _, k in ipairs(buffOrder) do
		buttons[k] = CreateBuffButton(
			buffTrackerFrame,
			{ 32, 32 },
			buffs[k]["buff1"]["name"],
			buffs[k]["buff2"] and buffs[k]["buff2"]["name"],
			buffs[k]["buff1"]["icon"],
			k
		)
		buttons[k]:Hide()
...
local function ShowMissingBuffs(unit)
	...
	if not missingBuffsFromMe[unit] then
		return
	end
...
local function CheckUnit(unit, updateBtn)
	...
	if not hasBuffProvider then
		return
	end

	if missingBuffsFromMe[unit] then
		wipe(missingBuffsFromMe[unit])
	end
		...
		for buff in pairs(available) do
			if required[buff] then
				...
				if exists then
					...
					-- Only add to missing buffs if player knows the spell
					if buffsProvidedByMe[buff] then
						local spellId = buffsProvidedByMe[buff][3]
						if not spellId or PlayerKnowsSpell(spellId) then
