...
local function InitIndicator(indicatorName)
	...
	elseif indicatorName == "externalCooldowns" then
		local icons = {
			...
			"Interface\\Icons\\Spell_Holy_DivineProtection",
...
local function UpdateIndicators(layout, indicatorName, setting, value, value2)
	if not indicatorsTab:IsShown() then
		return
	end
	...
	if not indicatorName then -- init
			...
			for i, t in pairs(currentLayoutTable["indicators"]) do
				...
				if t["position"] then
					if t["indicatorName"] == "statusText" then
						...
						local relativeTo = t["position"][2] == "healthBar" and previewButton.widgets.healthBar
							or previewButton
						P.Point(
							indicator,
							t["position"][1],
				...
				if t["num"] then
					...
					if indicator.UpdateSize then
						indicator:UpdateSize(t["num"])
					end
				end
				-- update format
				if t["format"] then
				...
				if t["color"] then
					if
						t["indicatorName"] == "nameText"
						or t["indicatorName"] == "healthText"
						or t["indicatorName"] == "powerText"
					then
				...
				if t["privateAuraOptions"] then
					...
					indicator.cooldown:SetHideCountdownNumbers(
						not (t["privateAuraOptions"][1] and t["privateAuraOptions"][2])
					)
				end
				-- update glow
		...
		elseif setting == "create" then
			...
			if value["position"] then
				P.ClearPoints(indicator)
				local relativeTo = value["position"][2] == "healthBar" and previewButton.widgets.healthBar
					or previewButton
				P.Point(
					indicator,
					value["position"][1],
			...
			if value["num"] then
				...
				if indicator.UpdateSize then
					indicator:UpdateSize(value["num"])
				end
			end
			-- update numPerLine
	...
	if masters[currentLayout] then
		...
		for _, layout in ipairs(indices) do
			tinsert(items, {
				...
				["value"] = layout,
				["onClick"] = function()
					local popup = Cell.CreateConfirmPopup(
						indicatorsTab,
						200,
						L["All indicators of %s will be replaced with those in %s"]:format(
							"|cffff0066" .. (currentLayout == "default" and _G.DEFAULT or currentLayout) .. "|r",
							"|cffff0066" .. (layout == "default" and _G.DEFAULT or layout) .. "|r"
						),
						function(self)
...
local function CreateListPane()
	...
	createBtn:SetScript("OnClick", function()
		local popup = Cell.CreateConfirmPopup(indicatorsTab, 220, L["Create new indicator"], function(self)
			...
			tinsert(
				currentLayoutTable["indicators"],
				I.GetDefaultCustomIndicatorTable(name, indicatorName, indicatorType, indicatorAuraType)
			)
			Cell.Fire(
				"UpdateIndicators",
				F.GetNotifiedLayoutName(currentLayout),
				indicatorName,
				"create",
				currentLayoutTable["indicators"][last]
			)
	...
	local importBtn = Cell.CreateButton(
		listPane,
		nil,
		"accent-hover",
		{ 46, 20 },
		...
		nil,
		L["Import"],
		L["Custom indicators will not be overwritten, even with same name"]
	)
	...
	local copyBtn = Cell.CreateButton(
		listPane,
		nil,
		"accent-hover",
		{ 46, 20 },
		...
		nil,
		L["Copy"],
		L["Copy indicators from one layout to another"],
		L["Custom indicators will not be overwritten, even with same name"]
	)
...
local DEBUFFS_TOOLTIP1 = L["This will make these icons not click-through-able"]
	.. "|"
	.. L["Tooltips need to be enabled in General tab"]
...
indicatorSettings = {
	...
	["statusText"] = {
		"enabled",
		"checkbutton:showTimer",
		"checkbutton2:showBackground",
		"statusColors",
		"statusPosition",
		"frameLevel",
		"font-noOffset",
	...
	["healthText"] = {
		"|cffff7727" .. L["MODERATE CPU USAGE"],
		"enabled",
		"healthFormat",
		"position",
		"frameLevel",
	...
	["statusIcon"] = {
		...
		"|TInterface\\LFGFrame\\LFG-Eye:18:18:0:0:512:256:72:120:72:120|t "
			...
			.. "|A:alliance_icon_and_flag-dynamicIcon:18:18|a ",
		"enabled",
		"size-square",
		"position",
		"frameLevel",
	...
	["shieldBar"] = {
		"enabled",
		"checkbutton:onlyShowOvershields",
		"color-alpha",
		"height",
		"shieldBarPosition",
		"frameLevel",
	...
	["powerWordShield"] = {
		L["To show shield value, |cffff2727Glyph of Power Word: Shield|r is required"],
		"enabled",
		"checkbutton:shieldByMe",
		"size-width",
		"position",
		"frameLevel",
	},
	["aoeHealing"] = {
		"|cffb7b7b7" .. L["Display a gradient texture when the unit receives a heal from your certain healing spells."],
		"enabled",
		"builtInAoEHealings",
		"customAoEHealings",
		"color",
	...
	["externalCooldowns"] = {
		L['Even if disabled, the settings below affect "Externals + Defensives" indicator'],
		"enabled",
		"builtInExternals",
		"customExternals",
		"durationVisibility",
	...
	["debuffs"] = {
		"enabled",
		"checkbutton:dispellableByMe",
		"debuffBlacklist",
		"bigDebuffs",
		"durationVisibility",
		"checkbutton2:showAnimation",
		"checkbutton3:showTooltip:" .. DEBUFFS_TOOLTIP1,
		"checkbutton4:enableBlacklistShortcut:" .. DEBUFFS_TOOLTIP2,
		"size-normal-big",
		"num:10",
		"orientation",
		"position",
	...
	["raidDebuffs"] = {
		"|cffb7b7b7"
			.. L["You can config debuffs in %s"]:format(Cell.GetAccentColorString() .. L["Raid Debuffs"] .. "|r"),
		"enabled",
		"checkbutton:onlyShowTopGlow",
		"durationVisibility",
		"checkbutton2:showTooltip:" .. DEBUFFS_TOOLTIP1,
		"size-border",
		"num:3",
		"orientation",
		"position",
	...
	["targetedSpells"] = {
		"enabled",
		"checkbutton:showAllSpells:" .. L["Glow is only available to the spells in the list below"],
		"targetedSpellsList",
		"targetedSpellsGlow",
		"size-border",
		"num:3",
	...
	["targetCounter"] = {
		"|cffff2727" .. L["HIGH CPU USAGE"] .. "!|r |cffb7b7b7" .. L["Check all visible enemy nameplates."],
		"enabled",
	...
	["missingBuffs"] = {
		"|cffb7b7b7" .. (L["%s in Utilities must be enabled to make this indicator work."]:format(
			Cell.GetAccentColorString() .. L["Buff Tracker"] .. "|r"
		)),
		"enabled",
		"size-square",
		"orientation",
...
local function ShowIndicatorSettings(id)
	...
	if indicatorType == "built-in" then
		...
		if indicatorType == "icon" then
			settingsTable = {
				"enabled",
				"auras",
				"checkbutton3:showStack",
				"durationVisibility",
				"checkbutton4:showAnimation",
				"glowOptions",
				CELL_RECTANGULAR_CUSTOM_INDICATOR_ICONS and "size" or "size-square",
				"position",
				"frameLevel",
				"font1:stackFont",
				"font2:durationFont",
		...
		elseif indicatorType == "text" then
			settingsTable =
				{ "enabled", "auras", "duration", "stack", "colors", "position", "frameLevel", "font-noOffset" }
		...
		elseif indicatorType == "overlay" then
			settingsTable =
				{ "enabled", "auras", "overlayColors", "checkbutton3:smooth", "barOrientation", "frameLevel:50" }
		...
		if indicatorType == "glow" then
			tinsert(
				...
				1,
				"|cffb7b7b7"
					.. L["The spells list of a icons indicator is unordered (no priority)."]
					.. " "
					.. L["Indicator settings are part of Layout settings which are account-wide."]
			)
		else
			tinsert(
				settingsTable,
				1,
				"|cffb7b7b7"
					.. L["The priority of spells decreases from top to bottom."]
					.. " "
					.. L["Indicator settings are part of Layout settings which are account-wide."]
			)
		end
	...
	for i, w in pairs(widgets) do
		...
		if currentSetting == "color-alpha" or currentSetting == "color-class" or currentSetting == "color-power" then
			currentSetting = "color"
		end
		if currentSetting == "customColors" or currentSetting == "overlayColors" or currentSetting == "blockColors" then
			currentSetting = "colors"
		end
		if currentSetting == "size-square" or currentSetting == "size-normal-big" then
			currentSetting = "size"
		end
		if
			currentSetting == "statusPosition"
			or currentSetting == "position-noHCenter"
			or currentSetting == "shieldBarPosition"
		then
			currentSetting = "position"
		end
		if currentSetting == "barOrientation" then
			currentSetting = "orientation"
		end
		if currentSetting == "durationVisibility" then
			currentSetting = "showDuration"
		end
		if currentSetting == "powerFormat" then
			currentSetting = "format"
		end
		...
		elseif currentSetting == "auras" then
			w:SetDBValue(
				L[F.UpperFirst(indicatorTable["auraType"]) .. " List"],
				indicatorTable["auras"],
				indicatorType == "glow",
				indicatorType == "icons",
		...
		elseif currentSetting == "size-width" then
			...
			w:SetFunc(function(value)
				...
				indicatorTable["size"] =
					{ value[1], (indicatorTable["size"] and indicatorTable["size"][2]) or value[2] or 0 }
		...
		elseif
			currentSetting == "targetCounterFilters"
			or currentSetting == "dispelFilters"
			or currentSetting == "powerTextFilters"
		then
...
LoadIndicatorList = function()
	...
	ListHighlightFn = Cell.CreateButtonGroup(listButtons, ShowIndicatorSettings, function(id)
		...
		if not currentLayoutTable["indicators"][id] then
			return
		end
