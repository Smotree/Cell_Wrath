...
local importTargetSpecID -- spec to import into (for dual spec support)
...
local function DoImport()
	if Cell.vars.clickCastings["useCommon"] then
		...
		-- Use importTargetSpecID if set, otherwise use current spec
		local specID = importTargetSpecID or Cell.vars.playerSpecID
		Cell.vars.clickCastings[specID] = imported
	end
...
local function CreateClickCastingImportExportFrame()
	importExportFrame =
		CreateFrame("Frame", "CellOptionsFrame_ClickCastingsImportExport", Cell.frames.clickCastingsTab, nil)
	importExportFrame:Hide()
	...
	-- close
	local closeBtn = Cell.CreateButton(
		importExportFrame,
		"X",
		"red",
		{ 18, 18 },
	...
	importBtn:SetScript("OnClick", function()
		...
		local popup = Cell.CreateConfirmPopup(
			Cell.frames.clickCastingsTab,
			200,
			L["Overwrite Click-Casting"] .. "?",
			function(self)
				DoImport()
			end,
			nil,
				...
				if class and version and data then
					if version >= Cell.MIN_CLICKCASTINGS_VERSION then
						...
						if success and data then
							title:SetText(
								L["Import"] .. ": " .. F.GetClassColorStr(class) .. F.GetLocalizedClassName(class)
							)
							imported = data
	...
	-- highlight text
	textArea.eb:SetScript("OnEditFocusGained", function()
...
-- Set target spec for import (used by dual spec support)
function F.SetClickCastingImportTargetSpec(specID)
	importTargetSpecID = specID
end
...
function F.ShowClickCastingExportFrame(clickCastingTable)
	...
	title:SetText(
		L["Export"]
			.. ": "
			.. F.GetClassColorStr(Cell.vars.playerClass)
			.. F.GetLocalizedClassName(Cell.vars.playerClass)
	)
