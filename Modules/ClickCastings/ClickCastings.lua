...
local selectedSpecID -- For WotLK: which spec to view/edit (1 or 2), nil = current
-------------------------------------------------
-- changes
-------------------------------------------------
...
wrapFrame:SetAttribute(
	"_onstate-mouseoverstate",
	[[
			...
			if key then
				...
				if key == "SCROLLUP" or key == "SCROLLDOWN" then
					...
					if not bindingClicks[key] then
						...
						bindingClicks[key] = [[self:SetBindingClick(true, "]]
							.. strupper(m .. k)
							.. [[", self, "]]
							.. key
							.. [[")]]
...
local function ClearClickCastings(b)
	if not previousClickCastings then
		return
	end
...
local function ApplyClickCastings(b)
	for i, t in pairs(clickCastingTable) do
		...
		if t[2] == "spell" then
			...
			local sMaRt = ""
			if
				smartResurrection ~= "disabled" and not (F.IsResurrectionForDead(spellName) or F.IsSoulstone(spellName))
			then
				...
				if strfind(smartResurrection, "combat$") then
					if F.GetCombatResurrection(Cell.vars.playerClass) then
						sMaRt = sMaRt
							...
							.. unit
							.. ",dead,combat] "
							.. F.GetCombatResurrection(Cell.vars.playerClass)
					end
				end
			end
...
function F.UpdateClickCastings(noReload, onlyqueued)
	...
	clickCastingTable = Cell.vars.clickCastings["useCommon"] and Cell.vars.clickCastings["common"]
		or Cell.vars.clickCastings[Cell.vars.playerSpecID]

	-- FIXME: remove this determine statement
	if Cell.vars.clickCastings["alwaysTargeting"] then
		alwaysTargeting =
			Cell.vars.clickCastings["alwaysTargeting"][Cell.vars.clickCastings["useCommon"] and "common" or Cell.vars.playerSpecID]
	else
...
local function LoadSpecProfile(specID)
	selectedSpecID = specID
	-- Load click castings for the selected spec
	clickCastingTable = Cell.vars.clickCastings[specID]
	if not clickCastingTable then
		Cell.vars.clickCastings[specID] = {}
		clickCastingTable = Cell.vars.clickCastings[specID]
	end
	-- Update alwaysTargeting for the selected spec
	if Cell.vars.clickCastings["alwaysTargeting"] then
		alwaysTargeting = Cell.vars.clickCastings["alwaysTargeting"][specID] or "disabled"
	else
		alwaysTargeting = "disabled"
	end
	LoadProfile(false, specID)
end
...
local function CreateProfilePane()
	...
	profileDropdown = Cell.CreateDropdown(profilePane, 200)
	profileDropdown:SetPoint("TOPLEFT", profilePane, "TOPLEFT", 5, -27)

	-- Spec selector dropdown (only for WotLK/Cata when using separate profiles)
	specDropdown = Cell.CreateDropdown(profilePane, 200)
	specDropdown:SetPoint("TOPLEFT", profileDropdown, "TOPRIGHT", 7, 0)
	specDropdown:Hide()

	local function UpdateSpecDropdown()
		if Cell.isWrath or Cell.isCata then
			local items = {}
			-- Primary Talents
			table.insert(items, {
				["text"] = L["Primary Talents"],
				["value"] = 1,
				["onClick"] = function()
					LoadSpecProfile(1)
				end,
			...
			specDropdown:SetSelectedValue(selectedSpecID or Cell.vars.playerSpecID)
		end
	end
	...
	profileDropdown:SetItems({
		...
		},
		{
			...
			["onClick"] = function()
				...
				selectedSpecID = Cell.vars.playerSpecID
				if Cell.isWrath or Cell.isCata then
					UpdateSpecDropdown()
					specDropdown:Show()
				end
				Cell.Fire("UpdateClickCastings")
				LoadProfile(false, selectedSpecID)
			end,
		},
	})

	-- Initialize on show
	clickCastingsTab:HookScript("OnShow", function()
		if Cell.vars.clickCastings["useCommon"] then
			specDropdown:Hide()
		elseif Cell.isWrath or Cell.isCata then
			selectedSpecID = selectedSpecID or Cell.vars.playerSpecID
			UpdateSpecDropdown()
			specDropdown:Show()
		end
	end)
...
local function CreateSmartResPane()
	...
	Cell.SetTooltips(
		...
		L["Replace click-castings of Spell type with resurrection spells on dead units"]
	)
end
	...
	local items = {
		{
			...
			["onClick"] = function()
				...
				if clickCastingsTab.popupEditBox then
					clickCastingsTab.popupEditBox:Hide()
				end
		...
		if Cell.isRetail or Cell.isMists then
			listPane:SetTitle(
				L["Current Profile"]
					.. ": "
					.. "|T"
					.. Cell.vars.playerSpecIcon
					.. ":12:12:0:1:12:12:1:11:1:11|t "
					.. Cell.vars.playerSpecName
			)
...
local function CreateListPane()
	...
	import:SetScript("OnClick", function()
		-- Pass selected spec to import function for dual spec support
		F.SetClickCastingImportTargetSpec(selectedSpecID)
	...
	saveBtn:SetScript("OnClick", function()
		...
		for index, b in pairs(deleted) do
			if changed[index] then
				changed[index] = nil
			end -- if duplicated in changed, remove it
...
LoadProfile = function(isCommon, specID)
	targetingDropdown:SetSelectedValue(alwaysTargeting)

	-- Update title based on common or spec-specific profile
	if isCommon then
		listPane:SetTitle(L["Current Profile"] .. ": " .. L["Common"])
	else
		-- For WotLK/Cata: show which spec we're editing
		if Cell.isWrath or Cell.isCata then
			local specName = (specID or selectedSpecID or Cell.vars.playerSpecID) == 1 and L["Primary Talents"]
				or L["Secondary Talents"]
			local isActive = (specID or selectedSpecID) == Cell.vars.playerSpecID
			local activeTag = isActive and "" or " |cff888888(" .. L["Inactive"] .. ")|r"
			listPane:SetTitle(L["Current Profile"] .. ": " .. specName .. activeTag)
		else
...
function F.UpdateClickCastingProfileLabel()
	if loaded then
		...
		-- When talents change, update selectedSpecID and refresh profile if not using common
		if not Cell.vars.clickCastings["useCommon"] and selectedSpecID ~= Cell.vars.playerSpecID then
			selectedSpecID = Cell.vars.playerSpecID
			if specDropdown then
				specDropdown:SetSelectedValue(selectedSpecID)
			end
			LoadProfile(Cell.vars.clickCastings["useCommon"])
		end
	end
