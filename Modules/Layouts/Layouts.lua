...
local function UpdatePreviewButton(which, value)
	...
	if not which or which == "nameText" then
		...
		if iTable["enabled"] then
			...
			previewButton.indicators.nameText:SetPoint(
				iTable["position"][1],
				relativeTo,
				iTable["position"][3],
				iTable["position"][4],
	...
	if not which or which == "barOrientation" then
		B.SetOrientation(
			previewButton,
			selectedLayoutTable["barOrientation"][1],
			selectedLayoutTable["barOrientation"][2]
		)
	end
...
local function UpdateLayoutPreview()
	...
	-- re-arrange
	local point, anchorPoint, groupAnchorPoint, unitSpacing, groupSpacing, _, _, verticalSpacing, horizontalSpacing =
		F.GetRaidFramePoints(selectedLayoutTable["main"])
	...
	if selectedLayoutTable["main"]["combineGroups"] and previewMode ~= 1 then
		...
		if selectedLayoutTable["main"]["orientation"] == "vertical" then
			layoutPreview.combinedHeader:SetSize(
				...
				P.Scale(selectedLayoutTable["main"]["size"][2]) * unitsPerColumn
					+ abs(unitSpacing) * (unitsPerColumn - 1)
			)
			...
			for i = 1, min(40, units) do
				...
				elseif i % selectedLayoutTable["main"]["unitsPerColumn"] == 1 then
					header[i]:SetPoint(
						point,
						header[i - selectedLayoutTable["main"]["unitsPerColumn"]],
						groupAnchorPoint,
						groupSpacing,
						0
					)
			...
			layoutPreview.combinedHeader:SetSize(
				P.Scale(selectedLayoutTable["main"]["size"][1]) * unitsPerColumn
					+ abs(unitSpacing) * (unitsPerColumn - 1),
		...
		for i = 1, 40 do
			P.Size(
				layoutPreview.combinedHeader[i],
				selectedLayoutTable["main"]["size"][1],
				selectedLayoutTable["main"]["size"][2]
			)
			if i > units then
		...
		for i, group in ipairs(shownGroups) do
			...
			if selectedLayoutTable["main"]["orientation"] == "vertical" then
				header:SetSize(
					P.Scale(selectedLayoutTable["main"]["size"][1]),
					P.Scale(selectedLayoutTable["main"]["size"][2]) * unitsPerColumn
						+ abs(unitSpacing) * (unitsPerColumn - 1)
				)
				...
				if i == 1 then
					...
					if headerCol == 1 then -- first column on each row
						header:SetPoint(
							point,
							layoutPreview.separatedHeaders[shownGroups[i - headersPerRow]],
							0,
							verticalSpacing
						)
					else
						header:SetPoint(
							point,
							layoutPreview.separatedHeaders[shownGroups[i - 1]],
							groupAnchorPoint,
							groupSpacing,
							0
						)
				...
				header:SetSize(
					P.Scale(selectedLayoutTable["main"]["size"][1]) * unitsPerColumn
						+ abs(unitSpacing) * (unitsPerColumn - 1),
					P.Scale(selectedLayoutTable["main"]["size"][2])
				)
				for j = 1, unitsPerColumn do
					P.Size(header[j], selectedLayoutTable["main"]["size"][1], selectedLayoutTable["main"]["size"][2])
					header[j]:ClearAllPoints()
				...
				if i == 1 then
					...
					if headerRow == 1 then -- first row on each column
						header:SetPoint(
							point,
							layoutPreview.separatedHeaders[shownGroups[i - headersPerCol]],
							horizontalSpacing,
							0
						)
					else
...
local function UpdatePetPreview()
	...
	if
		(
			not (selectedLayoutTable["pet"]["partyEnabled"] and selectedLayoutTable["pet"]["partyDetached"])
			and not selectedLayoutTable["pet"]["raidEnabled"]
		)
		or ((previewMode == 1) and not (selectedLayoutTable["pet"]["partyEnabled"] and selectedLayoutTable["pet"]["partyDetached"]))
		or ((previewMode == 2) and not selectedLayoutTable["pet"]["raidEnabled"])
	then
	...
	if orientation == "vertical" then
		...
		header:SetSize(
			P.Scale(width) * 4 + abs(P.Scale(unitSpacing)) * 3,
			P.Scale(height) * 5 + abs(P.Scale(unitSpacing)) * 4
		)
		header:SetPoint(point)
		...
		header:SetSize(
			P.Scale(width) * 5 + abs(P.Scale(unitSpacing)) * 4,
			P.Scale(height) * 4 + abs(P.Scale(unitSpacing)) * 3
		)
		header:SetPoint(point)
...
local function CreateSpotlightPreview()
	...
	for i = 1, 15 do
		...
		spotlightPreview.header[i].tex:SetPoint(
			"TOPLEFT",
			spotlightPreview.header[i],
			"TOPLEFT",
			P.Scale(1),
			P.Scale(-1)
		)
		...
		spotlightPreview.header[i].tex:SetVertexColor(
			F.ConvertRGB(255, 0, 102, i % 5 == 0 and desaturation[5] or desaturation[i - floor(i / 5) * 5])
		)
...
local function CreateLayoutPane()
	...
	renameBtn:SetScript("OnClick", function()
		local popup = Cell.CreateConfirmPopup(
			layoutsTab,
			200,
			L["Rename layout"] .. " " .. selectedLayout,
			function(self)
				local name = strtrim(self.editBox:GetText())
				if IsValidLayoutName(name) then
					-- update db
	...
	deleteBtn:SetScript("OnClick", function()
		local popup = Cell.CreateConfirmPopup(
			layoutsTab,
			200,
			L["Delete layout"] .. " " .. selectedLayout .. "?",
			function(self)
				-- update db
				CellDB["layouts"][selectedLayout] = nil
	...
	-- import
	local importBtn =
		Cell.CreateButton(layoutPane, nil, "accent-hover", { 33, 20 }, nil, nil, nil, nil, nil, L["Import"])
	...
	-- export
	local exportBtn =
		Cell.CreateButton(layoutPane, nil, "accent-hover", { 33, 20 }, nil, nil, nil, nil, nil, L["Export"])
		...
		typeSwitch = Cell.CreateSwitch(
			...
			function(value)
				if value == "role" then
					...
					CellDB["layoutAutoSwitch"][Cell.vars.playerClass][Cell.vars.playerSpecID] =
						F.Copy(CellDB["layoutAutoSwitch"]["role"][Cell.vars.playerSpecRole])
				end
...
local function CreateGroupFilterPane()
	...
	-- preview mode
	local previewModeButton =
		Cell.CreateButton(groupFilterPane, L["Preview"] .. ": |cff777777" .. L["OFF"], "accent", { 195, 20 })
...
local function CreateLayoutSetupPane()
	...
	sameArrangementAsMainCB = Cell.CreateCheckButton(
		layoutSetupPane,
	...
	-- combine groups
	combineGroupsCB = Cell.CreateCheckButton(
		pages.main,
		L["Combine Groups"] .. " (" .. L["Raid"] .. ")",
		function(checked, self)
	...
	-- unitsPerColumn
	unitsSlider = Cell.CreateSlider(
		L["Units Per Column"],
		pages.main,
		2,
		Cell.isRetail and 20 or 25,
		117,
		1,
		function(value)
	...
	spotlightCB = Cell.CreateCheckButton(
		pages.spotlight,
		...
		L["Spotlight Frame"],
		L["Show units you care about more in a separate frame"],
	...
	local spotlightOrientationText =
		spotlightOrientationDropdown:CreateFontString(nil, "OVERLAY", "CELL_WRATH_FONT_WIDGET")
...
LoadLayoutAutoSwitchDB = function()
	if Cell.isRetail then
		...
		if Cell.vars.layoutAutoSwitchBy == "spec" then
			currentProfileBox.text:SetText(
				"|T" .. Cell.vars.playerSpecIcon .. ":12:12:0:0:12:12:1:11:1:11|t " .. Cell.vars.playerSpecName
			)
		else
			currentProfileBox.text:SetText(
				F.GetDefaultRoleIconEscapeSequence(Cell.vars.playerSpecRole, 12) .. " " .. _G[Cell.vars.playerSpecRole]
			)
		end
	...
	elseif Cell.isCata or Cell.isWrath then
		...
		if Cell.vars.activeTalentGroup == 1 then
			currentProfileBox.text:SetText(
				"|TInterface\\AddOns\\Cell_Wrath\\Media\\Icons\\1:13|t " .. L["Primary Talents"]
			)
		else
			currentProfileBox.text:SetText(
				"|TInterface\\AddOns\\Cell_Wrath\\Media\\Icons\\2:13|t " .. L["Secondary Talents"]
			)
		end
		raid10Dropdown:SetSelectedValue(Cell.vars.layoutAutoSwitch["raid10"])
		raid25Dropdown:SetSelectedValue(Cell.vars.layoutAutoSwitch["raid25"])
...
local function UpdateLayoutAutoSwitch(layout, which)
	...
	if raidInstanceText then
		raidInstanceText:SetText(raidInstanceText.text)
	end
	if raidMythicText then
		raidMythicText:SetText(raidMythicText.text)
	end
	if raid10Text then
