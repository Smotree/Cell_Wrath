	...
	for sourceGUID, castInfo in pairs(casts) do
		if guid == castInfo["targetGUID"] then
			if castInfo["endTime"] > GetTime() then -- not expired
				...
				if
					not castsOnUnit[guid][castKey]["endTime"]
					or castsOnUnit[guid][castKey]["endTime"] > castInfo["endTime"]
...
local function CheckUnitCast(sourceUnit, isRecheck)
	if not UnitIsEnemy("player", sourceUnit) then
		if Cell.isDebug then
			print("|cffff0000[TargetedSpells]|r", sourceUnit, "is not enemy, skipping")
		end
		return
	end
	...
	-- WotLK: UnitCastingInfo returns: name, nameSubtext, text, texture, startTime, endTime, isTradeSkill, castID, notInterruptible
	-- startTime and endTime are in milliseconds
	local name, _, _, texture, startTimeMS, endTimeMS, _, _, notInterruptible = UnitCastingInfo(sourceUnit)
	if not name then
		-- UnitChannelInfo: name, nameSubtext, text, texture, startTime, endTime, isTradeSkill, notInterruptible
		name, _, _, texture, startTimeMS, endTimeMS, _, notInterruptible = UnitChannelInfo(sourceUnit)
		isChanneling = true
	end

	-- Convert to numbers if they are strings (WotLK compatibility)
	if startTimeMS then
	...
	if Cell.isDebug then
		print(
			"|cff00ff00[TargetedSpells]|r CheckUnitCast:",
			sourceUnit,
			"Name:",
			name or "nil",
			"Texture:",
	...
	if name then
		...
		if Cell.isDebug then
			print(
				"|cff00ff00[TargetedSpells]|r SpellId from mapping:",
				spellId or "nil",
				"showAllSpells:",
				showAllSpells and "true" or "false"
			)
		end
	end
		...
		for guid, unit in pairs(recheck) do
			if casts[guid] then
				...
				if casts[guid]["recheck"] >= 6 then
					...
					empty = false
					local recheckRequired = (not casts[guid]["targetUnit"] and UnitExists(unit .. "target"))
						or (casts[guid]["targetUnit"] and not UnitIsUnit(unit .. "target", casts[guid]["targetUnit"]))
...
-- events
-------------------------------------------------
eventFrame:SetScript("OnEvent", function(_, event, sourceUnit, spellName, spellRank, castId)
	-- Debug output
	if Cell.isDebug then
		print("|cff00ff00[TargetedSpells]|r Event:", event, "Unit:", sourceUnit or "nil", "Spell:", spellName or "nil")
	end
	...
	if sourceUnit and strfind(sourceUnit, "^soft") then
		return
	end
...
function I.CreateTargetedSpells(parent)
	local targetedSpells =
		CreateFrame("Frame", parent:GetName() .. "TargetedSpellsParent", parent.widgets.indicatorFrame)
