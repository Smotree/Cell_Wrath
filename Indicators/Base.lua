...
local StartGlow = {
	...
	["pixel"] = function(frame)
		PixelGlow_Start(
			frame,
			frame.glowOptions.color,
			frame.glowOptions.N,
			frame.glowOptions.frequency,
			frame.glowOptions.length,
			frame.glowOptions.thickness
		)
	end,
	...
	if frame._elapsed >= 0.1 then
		...
		if Cell.vars.iconDurationColors then
			if frame._remain < Cell.vars.iconDurationColors[3][4] then
				frame.duration:SetTextColor(
					Cell.vars.iconDurationColors[3][1],
					Cell.vars.iconDurationColors[3][2],
					Cell.vars.iconDurationColors[3][3]
				)
			elseif frame._remain < (Cell.vars.iconDurationColors[2][4] * frame._duration) then
				frame.duration:SetTextColor(
					Cell.vars.iconDurationColors[2][1],
					Cell.vars.iconDurationColors[2][2],
					Cell.vars.iconDurationColors[2][3]
				)
			else
				frame.duration:SetTextColor(
					Cell.vars.iconDurationColors[1][1],
					Cell.vars.iconDurationColors[1][2],
...
local function Icon_OnUpdate_ElapsedTime(frame, elapsed)
	...
	if frame._elapsedTime > frame._duration then
		frame._elapsedTime = frame._duration
	end
...
local function BarIcon_SetCooldown(frame, start, duration, debuffType, texture, count, refreshing)
	if duration == 0 then
		...
		frame._remain = nil
		frame._elapsed = nil
		-- WotLK: restore original icon visibility
		...
		if frame.showAnimation then
			...
			-- WotLK: hide original icon when cooldown icon is shown (no mask support)
			if Cell.isWrath or Cell.isVanilla then
			...
			frame.stack:SetParent(frame)
			-- WotLK: show original icon when animation is off
			if Cell.isWrath or Cell.isVanilla then
				frame.icon:SetAlpha(1)
			end
		end
...
local function BarIcon_ShowAnimation(frame, show)
	...
	if show then
		frame.cooldown:Show()
		-- WotLK: when animation is on, we'll hide frame.icon in SetCooldown
		-- (no mask support, so cooldown.icon replaces frame.icon)
	else
		frame.cooldown:Hide()
		-- WotLK: restore original icon visibility when animation is off
		if Cell.isWrath or Cell.isVanilla then
			frame.icon:SetAlpha(1)
		end
	end
...
local function Icons_UpdateSize(icons, numAuras)
	if not (icons.width and icons.orientation) then
		return
	end -- not init
...
local function Color_SetCooldown(color, start, duration, debuffType)
	...
	elseif color.type == "debuff-type" and debuffType then
		color.solidTex:SetVertexColor(
			CellDB["debuffTypeColor"][debuffType]["r"],
			CellDB["debuffTypeColor"][debuffType]["g"],
...
local function Color_SetColors(self, colors)
	...
	elseif colors[1] == "gradient-vertical" then
		self:SetScript("OnUpdate", nil)
		self.gradientTex:SetGradient(
			"VERTICAL",
			CreateColor(colors[2][1], colors[2][2], colors[2][3], colors[2][4]),
			CreateColor(colors[3][1], colors[3][2], colors[3][3], colors[3][4])
		)
...
local function QuickAssistBars_UpdateSize(bars, barsShown)
	if not (bars.width and bars.height) then
		return
	end -- not init
	...
	if overlay._elapsed >= 0.1 then
		...
		if overlay.colors[3][1] and overlay._remain <= overlay.colors[3][2] then
			if overlay.state ~= 3 then
				...
				overlay:SetStatusBarColor(
					overlay.colors[3][3][1],
					overlay.colors[3][3][2],
					overlay.colors[3][3][3],
					overlay.colors[3][3][4]
				)
			end
		elseif overlay.colors[2][1] and overlay._remain <= overlay._duration * overlay.colors[2][2] then
			if overlay.state ~= 2 then
				overlay.state = 2
				overlay:SetStatusBarColor(
					overlay.colors[2][3][1],
					overlay.colors[2][3][2],
					overlay.colors[2][3][3],
					overlay.colors[2][3][4]
				)
			end
